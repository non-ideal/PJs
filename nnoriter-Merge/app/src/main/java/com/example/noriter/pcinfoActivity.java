...
public class pcinfoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle bundle){
        ...
        TextView review_refresh = (TextView)findViewById(R.id.pcinfo_review_refresh);
        review_refresh.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){
                String reviewid = dialog.getName();
                if(reviewid != null){
                    adapter.addItem(1,index,reviewid,dialog.getDate(),dialog.getTitle(),dialog.getContent());
        ...
        adapter.addItem(1,index,"test1","2018-9-15","리뷰 테스트 1","리뷰 테스트 1-1");
        adapter.addItem(2,index,"test2","2018-9-16","리뷰 테스트 2","리뷰 테스트 2-1");
        adapter.addItem(3,index,"test3","2018-9-17","리뷰 테스트 3","리뷰 테스트 3-1");
